...
export class ImageService {
    ...
    async dlImage(url: string, path: string = 'dl_image/newpic.jpg') : Promise<any> {
        return this.SABYService.SABYAuthGet(url, {}, 'stream')
        .then(res => {
    ...
    async saveAllImages() : Promise<any> {
        let images: Image[] = await this.findAll();
        images.forEach(img => {
            this.dlImage(saby_forimg_url + img.sabyUrl, 'dl_image/' + img.id + '.jpeg');
        })
    }
}