...
import { AxiosRequestConfig, AxiosHeaders, AxiosError, AxiosResponse, ResponseType } from 'axios';
...
import { saby_auth_url, saby_nomenclature_balances_url, saby_nomenclature_list_url, saby_price_lists_url, saby_sales_points_url, saby_warehouses_url } from 'src/constants';
...
export class SABYService {
    ...
    GetSalesPoints(): Promise<SABYSalesPoint[]>{
        return this.SABYAuthGet(saby_sales_points_url)
        .then(res => res.data.salesPoints)
    }
    ...
    GetPriceLists(): Promise<SABYPriceList[]>{
        ...
        return this.SABYAuthGet(saby_price_lists_url, params)
        .then(res => res.data.priceLists)
    }

    async GetProductsFromPriceList(priceListId : string): Promise<SABYProduct[]>{
        const params = {
            pointId: process.env.COMPANY_ID,
            priceListId: priceListId,
            page: 0
        }
        let products: SABYProduct[] = [];
        let res;
        do{
    ...
    GetWarehouses(): Promise<SABYWarehouse[]>{
        ...
        return this.SABYAuthGet(saby_warehouses_url, params)
        .then(res => res.data.warehouses)
    }

    GetBalancesFromPriceLists(priceLists: SABYPriceList[]): Promise<SABYBalance[]> {
        const params = {
            companies: process.env.COMPANY_ID,
            priceListIds: '[' + priceLists.map((a: SABYPriceList, ind: number, arr)=> a.id ) + ']',
            warehouses: 246
        }
        return this.SABYAuthGet(saby_nomenclature_balances_url, params)
        .then(res => res.data.balances)
    }

    GetBalancesFromNomenclatures(nomenclatures: number[]): Promise<SABYBalance[]> {
        const params = {
            companies: process.env.COMPANY_ID,
    ...
    SABYAuthGet(link: string, params: {} = {}, responseType: ResponseType = 'json' ) : Promise<AxiosResponse> {
