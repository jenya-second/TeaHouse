...
export class ProductController {
    ...
    @Get('save')
    async saveProducts() {
        const SABYproducts: SABYProduct[] = await this.SABYService.GetProductsFromPriceList('10');
        let images: Image[] = [];
        ...
        SABYproducts.forEach((SABYproduct: SABYProduct)=>{
            ...
            images.push(...(SABYproduct.images.map((image: string)=>{
        ...
        return 'Saved ' + (await this.productService.saveMany(products)).length + ' products;\n' +
        'Saved ' + (await this.imageService.saveMany(images)).length + ' images;\n';
    }
